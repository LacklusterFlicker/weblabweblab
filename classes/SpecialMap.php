...
class SpecialMap extends BaseMap {
    ...
    {
        if ($id) {
        $res = $this->db->query("SELECT special_id, name, otdel_id "
    ...
    private function insert(Special $special) {
        ...
        if ($this->db->exec("INSERT INTO special(name, otdel_id)"
        . " VALUES($name, $special->otdel_id)") == 1) {
    ...
    public function findViewById($id = null) {
        if ($id) {
        $res = $this->db->query("SELECT special.special_id, special.name, otdel.name AS special"
