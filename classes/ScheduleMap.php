...
class ScheduleMap extends BaseMap {
    ...
    public function existsScheduleTeacherAndGruppa($schedule)
    {
    ...
    public function findByStudentId($id = null)
    {
        ...
        $arrDay['name'] = $day->name;
        $arrDay['gruppa'] = [];
        $gruppas = $this->findGruppasByDayStudent($id, $day->day_id);
        foreach ($gruppas as $gruppa) {
        $arrGruppa = [];
        $arrGruppa['name'] = $gruppa->name;
        $arrGruppa['schedule'] = $this->findByGruppasDayStudent($id, $day->day_id,$gruppa->gruppa_id);
        $arrDay['gruppa'][] = $arrGruppa;
        }
        $result[] = $arrDay;
        }
    ...
    public function findGruppasByDayStudent($studentId, $dayId) 
    {
        $res = $this->db->query("SELECT DISTINCT
        gruppa.gruppa_id, gruppa.name, subject.name FROM lesson_plan "
        . "INNER JOIN schedule ON
        lesson_plan.lesson_plan_id=schedule.lesson_plan_id "
        . "INNER JOIN gruppa ON
        lesson_plan.gruppa_id=gruppa.gruppa_id "
        . "INNER JOIN student ON student.gruppa_id = gruppa.gruppa_id INNER JOIN subject ON lesson_plan.subject_id = subject.subject_id "
        . "INNER JOIN user ON student.user_id = user.user_id WHERE student.user_id=$studentId AND schedule.day_id=$dayId ORDER BY gruppa.name");
        return $res->fetchAll(PDO::FETCH_OBJ);  
    }
    ...
    public function findByGruppasDayTeacher($teacherId, $dayId, $gruppaId)
    {
        ...
        public function findByGruppasDayStudent($studentId, $dayId, $gruppaId)
    {
        $res = $this->db->query("SELECT
        schedule.schedule_id,lesson_num.name AS
        lesson_num,subject.name AS subject, classroom.name AS
        classroom, user.lastname AS fio FROM lesson_plan "
        . "INNER JOIN schedule ON
        lesson_plan.lesson_plan_id=schedule.lesson_plan_id "
        . "INNER JOIN subject ON
        lesson_plan.subject_id=subject.subject_id INNER JOIN lesson_num 
        ON schedule.lesson_num_id=lesson_num.lesson_num_id INNER JOIN classroom ON
        schedule.classroom_id=classroom.classroom_id
        INNER JOIN gruppa ON gruppa.gruppa_id = lesson_plan.gruppa_id
        INNER JOIN student ON student.gruppa_id = gruppa.gruppa_id
        INNER JOIN user ON student.user_id = user.user_id
        WHERE student.user_id=$studentId AND schedule.day_id=$dayId AND lesson_plan.gruppa_id=$gruppaId ORDER BY schedule.lesson_num_id");
