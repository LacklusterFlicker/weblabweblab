...
class StudentMap extends BaseMap {
    ...
    public function findAll($ofset = 0, $limit = 30)
    {
        ...
        user.patronymic) AS fio, user.birthday, " . " gender.name AS gender, gruppa.name AS gruppa,
        role.name AS role, student.num_zach as num_zach FROM user INNER JOIN student ON
        user.user_id=student.user_id " . "INNER JOIN gender ON
        user.gender_id=gender.gender_id INNER JOIN gruppa ON
        student.gruppa_id=gruppa.gruppa_id" . " INNER JOIN role ON user.role_id=role.role_id LIMIT
        $ofset, $limit");
    ...
    {
        if ($id) {
        $res = $this->db->query("SELECT student.user_id, gruppa.name AS gruppa, student.num_zach as num_zach FROM student "
