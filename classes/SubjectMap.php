...
class SubjectMap extends BaseMap{
    ...
    {
        if ($id) {
        $res = $this->db->query("SELECT subject_id, name, otdel_id "
    ...
    private function insert(Subject $subject) {
        ...
        . " VALUES($name, $subject->otdel_id, $hours)") == 1) {
    ...
    private function update(Subject $subject)
    {
        $name = $this->db->quote($subject->name);
        $hours = $this->db->quote($subject->hours);
        if ( $this->db->exec("UPDATE subject SET name = $name, otdel_id = $subject->otdel_id, hours = $hours"
    ...
    public  function findAll($ofset = 0, $limit = 30)
    {
        ...
        . " FROM subject INNER JOIN otdel ON subject.otdel_id=otdel.otdel_id LIMIT $ofset,
        $limit");
