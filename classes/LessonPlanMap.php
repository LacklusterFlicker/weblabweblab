...
class LessonPlanMap extends BaseMap {
    //put your code here
    public function existsPlan($plan) 
    {
        $res = $this->db->query("SELECT lesson_plan_id FROM lesson_plan "
        . "WHERE gruppa_id = $plan->gruppa_id AND subject_id = $plan->subject_id AND user_id = $plan->user_id");
    ...
    {
        if ($id) {
        $res = $this->db->query("SELECT lesson_plan.lesson_plan_id, gruppa.name AS gruppa, subject.name AS subject, "
        . "subject.hours FROM lesson_plan INNER JOIN gruppa ON lesson_plan.gruppa_id=gruppa.gruppa_id "
        . "INNER JOIN subject ON lesson_plan.subject_id=subject.subject_id WHERE lesson_plan.user_id=$id");
